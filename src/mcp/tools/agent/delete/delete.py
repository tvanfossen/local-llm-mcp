    ...
    try:
        ...
        if not force and interaction_count > 10:
            return _create_error(
                "Deletion Blocked", 
                f"Agent '{agent_name}' has {interaction_count} interactions. "
        ...
        success_msg = _format_deletion_success(
            agent_name, agent_id, managed_files_count, 
            interaction_count, cleanup_results
        )
...
async def delete_all_agents(args: dict[str, Any] = None) -> dict[str, Any]:
    ...
    try:
        ...
        if not force:
            return _create_error(
                "Deletion Blocked",
                "This operation will delete ALL agents. Use 'force: true' to confirm."
            )
