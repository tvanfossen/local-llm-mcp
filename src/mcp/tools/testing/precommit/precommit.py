...
async def run_pre_commit(args: dict[str, Any] = None) -> dict[str, Any]:
    ...
    try:
        ...
        return _create_error("Pre-commit Not Installed", 
...
def _process_precommit_result(result, hook: str, all_files: bool) -> dict[str, Any]:
    ...
    if result.returncode == 0:
        ...
        return _create_success(f"✅ **Pre-commit Passed**\n"
                              f"🔍 Checked: {hook_name} on {scope}\n"
        ...
        return _create_error("Validation Failed", 
                           summary + f"\n```\n{output}\n```")