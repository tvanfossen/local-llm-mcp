            ...
            if result.get("success"):
                if result.get("type") == "tool_calls":
                    ...
                    if workspace_success:
                        # Extract and save the XML content from the tool call
                        workspace_call = None
                        for call in tool_calls:
                            if call.get('tool_name') == 'workspace':
                                workspace_call = call
                                break

                        if workspace_call and 'content' in workspace_call.get('arguments', {}):
                            xml_content = workspace_call['arguments']['content']
                            self.logger.info(f"💾 Saving XML content to metadata file: {len(xml_content)} chars")

                            # Save the actual XML file structure to .meta file
                            final_metadata_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by agent {self.state.agent_id} at {timestamp} -->
<!-- Request: {request.message} -->
<!-- Status: completed -->
{xml_content}"""
                            with open(meta_file, 'w') as f:
                                f.write(final_metadata_xml)
                        else:
                            self.logger.warning("⚠️ No XML content found in workspace tool call")
