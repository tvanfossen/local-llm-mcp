...
from schemas.agent_schemas import AgentState
...
class AgentRegistry:
    ...
    def create_agent(
        ...
        managed_file: str,
        initial_context: str = ""
    ) -> Tuple[bool, Optional[Agent], Optional[str]]:
        """
    ...
    def get_agent_by_file(self, filename: str) -> Optional[Agent]:
    ...
    def delete_agent(self, agent_id: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            error_msg = f"Failed to delete agent {agent_id}: {str(e)}"
            logger.error(error_msg)
            return False, error_msg
    
    def check_file_conflict(self, filename: str, exclude_agent_id: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        """
    ...
    def get_registry_stats(self) -> Dict[str, any]:
        ...
        if active_agents > 0:
            avg_success_rate = sum(agent.state.success_rate for agent in self.agents.values()) / active_agents
    ...
    def repair_registry_integrity(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            error_msg = f"Failed to repair registry integrity: {str(e)}"
            logger.error(error_msg)
            return False, [error_msg]
